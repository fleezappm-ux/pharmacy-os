...
function openFromDateParam(){
  ...
  const target=dailyReports.find(n=>n.date&&String(n.date).slice(0,10)===targetDate);
  if(target)openDailyForm(target);
}
...
requireEditPermission("canEditDaily",()=>{loadDailyReports();loadOwnName()});
