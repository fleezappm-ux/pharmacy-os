...
const MODAL_IDS=["notice-modal","license-modal","pharmacist-modal","seller-modal","institution-modal","survey-modal","delete-modal"];
...
function renderSurveyList(){const c=document.getElementById("survey-edit-list");c.textContent="";if(!surveyRows.length){c.innerHTML='<p class="empty-message">処方箋調べのデータはありません。</p>';return}surveyRows.slice().sort((a,b)=>parseYearMonth(a.month)-parseYearMonth(b.month)).forEach(n=>{const row=document.createElement("button");row.type="button";row.className="list-row";const main=document.createElement("div");main.className="list-main";const title=document.createElement("span");title.className="list-title";title.textContent=n.month||"年月未設定";main.appendChild(title);const filled=n.count!==null&&n.count!==undefined&&n.count!=="";const detail=document.createElement("span");detail.className="list-detail";detail.textContent=filled?`処方箋受付回数：${n.count} ／ 合計：${text(n.total)}`:"未入力";main.appendChild(detail);const badge=document.createElement("span");badge.className=`badge ${filled?"green":"none"}`;badge.textContent=filled?"入力済み":"未入力";row.append(main,badge);row.addEventListener("click",()=>openSurveyForm(n));c.appendChild(row)})}

function openSurveyForm(n){document.getElementById("survey-modal-title").textContent=n?"処方箋調べを編集":"新規月を追加";document.getElementById("survey-id").value=n?.id||"";document.getElementById("survey-month").value=n?.month||"";document.getElementById("survey-count").value=n?.count??"";document.getElementById("survey-total").value=n?.total??"";document.getElementById("survey-daily-average").value=n?.dailyAverage??"";document.getElementById("survey-dental").value=n?.dental??"";document.getElementById("survey-ophthalmology").value=n?.ophthalmology??"";document.getElementById("survey-ent").value=n?.ent??"";document.getElementById("survey-other").value=n?.other??"";const btn=document.getElementById("survey-delete-button");btn.hidden=!n;btn.onclick=n?()=>openDeleteConfirm("survey",{id:n.id,type:n.month,order:null}):null;showModal("survey-modal")}
...
document.getElementById("survey-form").addEventListener("submit",async e=>{
  ...
  const payload={id:document.getElementById("survey-id").value,row:{
    ...
    "合計（処方箋受付枚数）":numOrEmpty("survey-total"),
    "1日平均取扱処方箋枚数":numOrEmpty("survey-daily-average"),
...
/* ===== 削除確認（共通） ===== */
const DELETE_LABELS={notice:"この届出を削除しますか？",license:"この許可・登録を削除しますか？",pharmacist:"この薬剤師を削除しますか？",seller:"この登録販売者を削除しますか？",institution:"この試験検査機関を削除しますか？",survey:"この月のデータを削除しますか？"};
const DELETE_ACTIONS={notice:"deleteStatusNotice",license:"deleteStatusLicense",pharmacist:"deleteStatusPharmacist",seller:"deleteStatusSeller",institution:"deleteStatusTestInstitution",survey:"deleteStatusSurveyRow"};
const DELETE_MODALS={notice:"notice-modal",license:"license-modal",pharmacist:"pharmacist-modal",seller:"seller-modal",institution:"institution-modal",survey:"survey-modal"};
